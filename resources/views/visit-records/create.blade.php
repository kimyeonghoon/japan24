    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                if (distance <= 100) {
                    statusDiv.innerHTML = `<span class="text-success">✓ 위치 확인 완료 (${Math.round(distance)}m)</span>`;
                    submitBtn.disabled = false;
                } else {
                    statusDiv.innerHTML = `<span class="text-danger">✗ 성에서 너무 멀리 떨어져 있습니다 (${Math.round(distance)}m)</span>`;
...
document.getElementById('photos').addEventListener('change', function(e) {
    ...
    if (files.length < 3) {
        previewDiv.innerHTML = '<div class="alert alert-warning">최소 3장의 사진이 필요합니다</div>';
        return;
    }
